...
%determine the fastest path to miners, and prioritise these paths when disseminating transactions.
Next, we discuss the ranking process.

%Our approach encompasses three changes to the protocol. The first consists in saving for each block that we receive from a neighbour the time that neighbour took to disseminate the block to us. 

%The second one is maintaining a list, for each neighbour, with the transactions we sent to him and how much time it took for those transactions to be added to a block. 

%Finally, the third change is to locally determine which path is better for sending our transactions so that they reach the miners faster and give priority to these paths in the dissemination process. Note that once a transaction is added to a block the epidemic dissemination process becomes irrelevant.
...
%This is achieved by  classifying neighbours that are closer to minres or that are miners themselves classification system we discuss next. the following classification system, which we discc achieve this, a node classifies its neighbours, and attributes to each one a ranking with descending order of classification (best nodes at the top). The neighbour classification is given by:
...
%our algorithm to adapt by preventing nodes that only generate a block once in a while from having a good classification eternally.
...
%In Section~\ref{sec:evaluation} we will discuss different configurations tested.
...
%, we were only able to identify some of the changes done to the current client by analyzing the source code thoroughly. 
...
%hope to develop an improvement to membership protocol with the help of information gathered to build more efficient paths.
